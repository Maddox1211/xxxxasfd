    ...
    async def setup_hook(self) -> None:
        for record in store.records.values():
            ...
            if status == "not_appealed":
                self.add_view(
                    AppealButtonView(record["punishment_id"])
                )
            elif status == "pending":
                self.add_view(
                    AppealDecisionView(record["punishment_id"])
                )
...
async def process_expired_softbans() -> None:
    ...
    for record in list(store.records.values()):
        ...
        unban_at = datetime.fromisoformat(
            record["unban_at"]
        )
        ...
        print(
            f"TEST: Auto-unban due for "
            f"{record['user_tag']} "
            f"(`{record['user_id']}`) in guild "
            f"`{record['guild_id']}`"
        )
...
class AppealModal(
    ...
    async def on_submit(
        ...
        if (
            record is None
            or record["appeal_status"] != "not_appealed"
        ):
...
class AppealButtonView(discord.ui.View):
    ...
    async def on_click(
        ...
        await interaction.response.send_modal(
            AppealModal(self.punishment_id)
        )
...
async def send_public_appeal_review(
    *,
    ...
    if channel is None or not isinstance(
        ...
        print(
            "Public reviewed bans channel could not be found."
        )
        return
...
async def help_command(
    ...
    embed.add_field(
        name="/moderate",
        value=(
            "Moderate a user with a ban, softban, "
            "kick, or timeout."
        ),
        inline=False,
    )
...
async def moderate(
    ...
    if (
        action.value == "softban"
        and duration_value in SOFTBAN_UNBAN_DELTAS
    ):
        unban_at = (
...
@bot.tree.command(
    name="exportrecords",
    description=(
        "Export a user's punishment records as a JSON file."
    ),
)
@app_commands.describe(
    user="User mention, username, or Discord ID.",

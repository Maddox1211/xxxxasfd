...
MODERATOR_FORUM_ID = 1530487797490647140
APPEAL_REVIEW_CHANNEL_ID = 1535769810346971309
...
TIMEOUT_APPEAL_DURATIONS = {"1d", "7d", "30d", "permanent"}
...
class ModerationBot(commands.Bot):
    def __init__(self) -> None:
        ...
        super().__init__(
            command_prefix="!",
            intents=intents,
            description="A freshie you dont gank today is a freshie that ganks you tomorrow."
        )
    ...
    async def setup_hook(self) -> None:
        for record in store.records.values():
            ...
            if status == "not_appealed":
                self.add_view(AppealButtonView(record["punishment_id"]))
            elif status == "pending":
                self.add_view(AppealDecisionView(record["punishment_id"]))
...
class PunishmentStore:
    ...
    async def update(self, punishment_id: str, **fields) -> None:
        async with self.lock:
...
def is_appeal_eligible(action: str, duration: str) -> bool:
    if action in ("perm_ban", "softban"):
        return True

    if action == "timeout" and duration in TIMEOUT_APPEAL_DURATIONS:
        return True
...
async def process_expired_softbans() -> None:
    ...
    for record in list(store.records.values()):
        if record["action"] != "softban":
            continue

        if record.get("unbanned") or not record.get("unban_at"):
            continue

        unban_at = datetime.fromisoformat(record["unban_at"])
        ...
        print(
            f"TEST: Auto-unban due for {record['user_tag']} "
            f"(`{record['user_id']}`) in guild `{record['guild_id']}`"
        )
...
async def resolve_user(
    ...
    if target.isdigit():
        try:
            return await bot.fetch_user(int(target))
        except discord.NotFound:
            return None
...
async def resolve_target(guild: discord.Guild, user: str) -> Optional[discord.User]:
    target = await resolve_user(guild, user)

    if target is None and user.isdigit():
        try:
...
async def create_moderation_report(
    *,
    ...
    embed.add_field(
        name="Proof",
        value=proof_text or "No proof text provided.",
        inline=False,
    )
...
def build_appeal_embed(record: dict, appeal_reason: str) -> discord.Embed:
    embed = discord.Embed(
    ...
    embed.add_field(
        name="Proof",
        value=record.get("proof_text") or "No proof text provided.",
        inline=False,
    )
...
class AppealModal(discord.ui.Modal, title="Submit Appeal"):
    ...
    def __init__(self, punishment_id: str) -> None:
        super().__init__()
        self.punishment_id = punishment_id

    async def on_submit(self, interaction: discord.Interaction) -> None:
        record = store.get(self.punishment_id)

        if record is None or record["appeal_status"] != "not_appealed":
        ...
        channel = bot.get_channel(APPEAL_REVIEW_CHANNEL_ID)

        if channel is None:
            await interaction.response.send_message(
                "Your appeal could not be submitted. The review channel is unavailable.",
                ephemeral=True,
            )
            return

        embed = build_appeal_embed(record, str(self.reason))
        view = AppealDecisionView(self.punishment_id)
...
class AppealButtonView(discord.ui.View):
    def __init__(self, punishment_id: str) -> None:
        ...
        button = discord.ui.Button(
            ...
            custom_id=f"appeal_button:{punishment_id}",
        )
...
class AppealDecisionView(discord.ui.View):
    def __init__(self, punishment_id: str) -> None:
        ...
        select = discord.ui.Select(
            ...
            custom_id=f"appeal_decision:{punishment_id}",
            options=[
                discord.SelectOption(label="Accept", value="accepted"),
                discord.SelectOption(label="Deny", value="denied"),
            ],
        )
    ...
    async def on_select(self, interaction: discord.Interaction) -> None:
        ...
        if member is None or not has_authorized_role(member):
            await interaction.response.send_message(
        ...
        target_user = bot.get_user(record["user_id"])

        if target_user is not None:
            try:
                await target_user.send(f"Your appeal has been {decision}.")
...
async def help_command(
    ...
    embed.add_field(
        name="/moderate",
        value="Moderate a user with a ban, softban, kick, or timeout.",
        inline=False,
    )
    embed.add_field(
        name="/cacheusers",
        value="Refresh the cached server users.",
        inline=False,
    )
    embed.add_field(
        name="/userid",
        value="Look up a user's Discord ID from the cached usernames.",
        inline=False,
    )
    embed.add_field(
        name="/lookupuser",
        value="View a user's punishment history.",
        inline=False,
    )
    embed.add_field(
        name="/exportrecords",
        value="Export a user's punishment records as a JSON file.",
        inline=False,
    )
    embed.add_field(
        name="/resetappeal",
        value="Reset appeal eligibility for one or all punishments belonging to a user.",
        inline=False,
    )
...
@app_commands.describe(
    ...
    duration="Duration of the action. Not used for kicks.",
...
async def moderate(
    ...
    if action.value == "kick":
        ...
        duration_label = duration.name

    target = await resolve_target(interaction.guild, user)
    ...
    eligible = is_appeal_eligible(action.value, duration_value)

    unban_at = None

    if action.value == "softban" and duration_value in SOFTBAN_UNBAN_DELTAS:
        unban_at = (
            datetime.now(timezone.utc) + SOFTBAN_UNBAN_DELTAS[duration_value]
        ).isoformat()
    ...
    record = await store.add(
        ...
        reason=reason,
        proof_text=proof_text,
        proof_url=proof.url if proof is not None else None,
        report_thread_id=thread.id if thread is not None else None,
        appeal_status="not_appealed" if eligible else "not_eligible",
        unban_at=unban_at,
        unbanned=False if unban_at is not None else None,
    )
    ...
    if eligible:
        ...
        dm_embed.add_field(name="Punishment", value=action.name, inline=True)
        dm_embed.add_field(name="Duration", value=duration_label, inline=True)
        dm_embed.add_field(name="Reason", value=reason, inline=False)
        dm_embed.add_field(
            name="Appeal",
            value="You may submit one appeal for this punishment using the button below.",
            inline=False,
        )
        ...
        try:
            await target.send(
                embed=dm_embed,
                view=AppealButtonView(record["punishment_id"]),
            )
            appeal_note = "\nAppeal notice sent to user."
        except (discord.Forbidden, discord.HTTPException):
            appeal_note = "\nCould not DM the user an appeal notice."

    await interaction.followup.send(
        result + report_note + appeal_note + kick_note,
        ephemeral=True,
    )
...
@bot.tree.command(
    name="userid",
    description="Look up a user's ID from a cached username.",
)
@app_commands.describe(
...
async def userid(
    ...
    matches = sorted(
        (
            (user_id, name)
            for user_id, name in bot.user_cache.items()
...
@bot.tree.command(
    name="lookupuser",
    description="Look up all punishment records for a user.",
)
@app_commands.describe(
    user="User mention, username, or Discord ID.",
)
...
async def lookupuser(
    ...
    records = store.for_user(target.id)
    ...
    embed = discord.Embed(
        ...
        description=f"Total punishments: {len(records)}",
        color=discord.Color.blue(),
        timestamp=datetime.now(timezone.utc),
    )
    ...
    for record in records[:10]:
        ...
        if record.get("unban_at"):
            unban_line = (
                ...
                else f"Unban scheduled: {record['unban_at'][:10]}"
            )
            value += f"\n{unban_line}"
    ...
    if len(records) > 10:
        embed.set_footer(text=f"Showing 10 of {len(records)} records.")
...
@bot.tree.command(
    name="exportrecords",
    description="Export a user's punishment records as a JSON file.",
)
@app_commands.describe(
    user="User mention, username, or Discord ID.",
)
...
async def exportrecords(
    ...
    filename = f"punishments_{target.id}.json"

    await interaction.followup.send(
        content=f"Exported {len(records)} record(s) for {target}.",
...
@bot.tree.command(
    name="resetappeal",
    description="Reset appeal eligibility for a user's punishment.",
)
@app_commands.describe(
    user="User mention, username, or Discord ID.",
    punishment_id="Specific punishment ID to reset. Leave blank to reset all.",
)
@admin_only()
async def resetappeal(
    ...
    records = [
        record
        for record in store.for_user(target.id)
        if record["appeal_status"] in ("pending", "accepted", "denied")
    ]
    ...
    if punishment_id is not None:
        records = [
            ...
            if record["punishment_id"] == punishment_id
        ]
    ...
    for record in records:
        ...
        bot.add_view(AppealButtonView(record["punishment_id"]))

    await interaction.followup.send(
        f"Reset {len(records)} appeal record(s) for {target}.",
        ephemeral=True,
    )
